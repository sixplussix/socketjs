...
function fake_response(req, res) {
  ...
  res.writeHead = function (status) {
    ...
    var r = [];
    r.push('HTTP/' + req.httpVersion + ' ' + status + ' ' + STATUS_CODES[status]);
...
function generateHandler(app, dispatcher) {
  return function (req, res, head) {
    ...
    objectExtend(req, parse(req.url, true));
...
function sockjsVersion() {
  var pkg = void 0;
  try {
    pkg = readdirSync(__dirname + '/../package.json', 'utf-8');
  } catch (x) {
    x;
  }
  if (pkg) {
